...
| 0.4 | `--watch` boucle sur un job natif échoué : finaliser `progress.json` (`finish(false, …)`) sur le chemin d'erreur de `run_install_job`, et faire sortir `watch()` aussi sur `exit_code` présent | b6 (repris de cb le 18/09) | S | `native.rs`, `main.rs`, `progress.rs` | test : un job dont le spec est invalide fait sortir `--watch` avec le message d'erreur | **fait** (b6) : `run_install_job_in` finalise `progress.json` (`finished=false`, message) sur toute erreur ; `--watch` sort aussi dès que le wrapper a écrit `exit_code`, affiche la fin du log si aucune progression n'a été publiée, et rend le code du job ; test |
| 0.5 | Spec de job `install-spec-<pid>.json` : un nom unique par appel (job id ou horodatage + compteur), supprimé par le job après lecture | b6 (repris de cb le 18/09) | S | `native.rs` | test : deux appels rapprochés produisent deux specs distincts | **fait** (b6) : `spec_file()` = pid + nanosecondes + compteur ; le job copie le spec dans son répertoire (`spec.json`) puis supprime l'original ; test |
...
| 1.9 | Pilotage d'Update Manager : tuer le **groupe** de processus au timeout de `run_console`, drainer la sortie après la fin ; `SumWorksOnThisDir.lock` dans `stale_locks`, test de vivacité avant suppression ; `selectedFixes` vide = erreur (« Empty selectedFixes ») : validation, description, signature `diagnose_log` ; `fix_script_generate` en batch ; `backupDeletePeriod` / `periodType`, `installSP` / `spKey` / `diagnoserKey` exposés ; `fixes_installed` en job plutôt que bloquant 600 s | b6 | M | `runner.rs` (`run_console` seulement), `sum.rs`, `wm-sum-mcp/tools.rs`, `diag.rs` (une signature) | tests ; vérifié : un timeout ne laisse ni JVM ni verrou | **fait en partie** (b6) : `run_console` tue le groupe de processus (SIGTERM puis SIGKILL) et draine la sortie après la fin ; `stale_locks` inclut `SumWorksOnThisDir.lock` et sonde le verrou POSIX (`held`), `sum_locks` ne supprime jamais un verrou tenu, `fix_run` lit `installDir` du script pour vérifier ce verrou ; `selectedFixes` vide = erreur de validation pour install/uninstall + signature `sum-empty-selected-fixes` ; `fix_script_generate` accepte `steps` (batch) et les clés `backupDeletePeriod`, `periodType`, `installSP`, `spKey`, `diagnoserKey`, `useSSL`. Reste : `fixes_installed via_sum` en job plutôt que bloquant |
...
- 2026-09-18 : 0.4 et 0.5 faits par `b6`. Point explicitement **non vérifié** faute d'identifiants IBM dans les deux sessions : que `fixes_available` ne repropose plus les 44 fixes enregistrés (1.1) ; à lancer par l'utilisateur, la clé pouvant être posée une fois avec `credential_set name=empower.key`.
